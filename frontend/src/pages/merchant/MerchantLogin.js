...
export default function MerchantLogin() {
  ...
  const submit = async e => {
    ...
    try {
      const { data } = await api.post('/auth/merchant/login', form);
      login(data.token);
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'radial-gradient(ellipse at 60% 30%, rgba(255,165,2,0.08) 0%, var(--bg) 70%)' }}>
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'16px', padding:'2.5rem', width:'100%', maxWidth:'380px' }}>
        <div style={{ textAlign:'center', marginBottom:'2rem' }}>
          ...
          <h2 style={{ fontFamily:'var(--font-head)', fontSize:'1.7rem', fontWeight:800 }}>Merchant Login</h2>
          <p style={{ color:'var(--text2)', fontSize:'0.85rem', marginTop:'0.3rem' }}>Manage your products & sales</p>
        </div>
        ...
        <p style={{ textAlign:'center', marginTop:'1.2rem', color:'var(--text2)', fontSize:'0.9rem' }}>
          New merchant? <Link to="/merchant/register">Register here</Link>
        </p>
