...
async function getCreditInfo(customerId, email) {
  const app = await Application.findOne({ email: email.toLowerCase(), status: 'approved' });
  ...
  // Get all repayments by this customer
  ...
  const total_spent  = purchases.reduce((sum, t) => sum + (t.amount || 0), 0);
  const total_repaid = repayments.reduce((sum, t) => sum + (t.amount || 0), 0);
...
router.get('/application', protect(['customer']), async (req, res) => {
  try {
    const app = await Application.findOne({ email: req.user.email.toLowerCase() });
    res.json(app);
  } catch (e) { res.status(500).json({ message: e.message }); }
});
...
router.post('/purchase', protect(['customer']), async (req, res) => {
  try {
    ...
    const product = await Product.findById(product_id).populate('merchant_id', 'businessname');
    if (!product)
    ...
    if (product.price > info.available_credit)
      return res.status(400).json({
        message: `Insufficient credit. Available: Rs.${info.available_credit.toFixed(2)}, Required: Rs.${product.price}`
      });

    const txn = await Transaction.create({
      customer_id: req.user.id,
      merchant_id: product.merchant_id._id || product.merchant_id,
    ...
    // Return updated product with merchant info
    res.json({
      message: `Payment successful for "${product.product_name}"`,
      txn,
      product: {
...
router.post('/repay', protect(['customer']), async (req, res) => {
  try {
    ...
    res.json({ message: `Repayment of Rs.${amount.toFixed(2)} successful!`, txn });
  } catch (e) { res.status(500).json({ message: e.message }); }
});
